#!/usr/bin/python
"""
Testing for bad characters. "\x00" is always a bad character, we will alway omitt this!

2 methods for byte array: 
    1.) this link: "https://gist.githubusercontent.com/int0x33/8ee831f599906b9f4c1c24616bf5440e/raw/9311ab7ae4d878bd8cbdd0ca894397562e21240c/bytearray"
    2.) In immunity set working directory by running the following:
            !mona config -set workingfolder c:\mona\%p
    2a.) Create byte array in monay:
            !mona bytearray -cpb \x00
Finding bad chars:
    1.) send exploit and crash program
    2.) compare esp against our generated byte array:
            !mona compare -a esp -f C:\mona\<Name of Program>\bytearray.bin
**note** if bad characters appear you can continue to omit them by using the following:**
     2a.)   !mona bytearray -cpb '<\x00\x00\x00>' #if you have a lot of characters

    3.) re run compare:
              !mona compare -a esp -f C:\mona\<Name of Program>\bytearray.bin
    **and we should get no errors**
    !!! Hooray, normal shellcode unmodified !!!
Worst case scenario do it manaully in the dump.

*****WORST CASE****
If the program errors out on the following comparison to the initial set of identified bad chars
are the bad chars you want ommit in your payload with msfvenom.
